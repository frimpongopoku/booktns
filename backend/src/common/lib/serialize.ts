...
export function serializeVendor(vendor: PrismaVendor): Vendor {
  return {
    ...
    suspendedReason: vendor.suspendedReason ?? undefined,
    videoSectionTitle: vendor.videoSectionTitle ?? undefined,
    videoSectionSubtitle: vendor.videoSectionSubtitle ?? undefined,
    galleryImageUrls: vendor.galleryImageUrls,
    galleryTitle: vendor.galleryTitle ?? undefined,
    galleryDescription: vendor.galleryDescription ?? undefined,
...
export function serializeOrder(order: PrismaOrder & { items: PrismaOrderItem[]; paymentMethod?: PrismaPaymentMethod | null }): Order {
  return {
    ...
    status: order.status,
    confirmationPdfUrl: order.confirmationPdfUrl ?? undefined,
